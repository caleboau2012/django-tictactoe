...
class Game(models.Model):
    ...
    def board(self):
        """
        Return a 2 dimentional list of Move objects so you can ask for the state of a square at [y, x]
        """
    ...
    def _get_game_status_after_move(self, move):
        ...
        board = self.board()
        if (
            (move == board[y][0] == board[y][1] == board[y][2])
            or (move == board[0][x] == board[1][x] == board[2][x])
            or (move == board[0][0] == board[1][1] == board[2][2])
            or (move == board[0][2] == board[1][1] == board[2][0])
        ):
